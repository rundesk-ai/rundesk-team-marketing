...
| The same conditional for the other three members | Scout, Signal, and Quill each still open with an unconditional instruction to read the worked-on repository's `AGENTS.md`. Scout is an external researcher and, like Beacon, will often have no repository at all | An owner decision on whether the team convention should change with it. Beacon was changed alone because Beacon is the member whose runs exposed the behavior |
