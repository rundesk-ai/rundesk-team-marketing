...
| AI citation counts on Microsoft surfaces | The shared SEO lifecycle accepts a dated Bing Webmaster Tools AI Performance UI observation or supplied export and refuses to relabel ordinary Bing search metrics as citations | A supported API for that report. Microsoft documents the UI report but no report-specific API, so a general Bing Webmaster package would not close this gap |
| Which AI crawler fetched what | The shared SEO lifecycle defines a supplied server-log evidence contract, including provenance, period, parser, row reconciliation, and crawler-identity verification | Direct read-only access to the property's actual log source; no universal provider exists |
| Query demand and volume | Search Console reports only queries the property already ranks for, so it cannot show an absent one. The shared lifecycle requires a named demand source and treats estimates separately from business value | A demand integration. Google Ads Keyword Planner is one candidate, but it requires a Google Ads customer, developer token, and a broader OAuth scope that this catalog does not declare |
...
| Direct Search Console bulk-export access in BigQuery | The Google catalog documents the owner-only, non-retroactive setup and supplied-query handoff. This team still has no BigQuery grant, so Beacon can verify a supplied extract but cannot query the warehouse directly |
...
| Any enforcement of the file-integrity checks | [`verifying-datasets`](../../skills/verifying-datasets/SKILL.md) fixes the profile, check list, and reproducible local-processing trail so two verifications of one file can agree. It remains guidance a member may follow rather than something that runs, and the 2026-08-25 runs showed the cost: one run counted columns with a naive split and reported its own parser as a defect in the data, and two runs read the same round row count without raising truncation. The guidance now names a quote-aware reader and requires the exact transformation; nothing enforces either | A package that profiles a file and returns the check set as structured output. Guidance closes the wording gap; only a package closes the enforcement one, which is the same distinction the Beacon retrieval row draws |

## Capabilities no member owns

| Missing | Consequence |
|---|---|
| Paid, email, lifecycle, organic-social acquisition strategy, affiliate, and marketplace methods | The domain caller may apply the SEO method to organic and AI-search planning only. Quill can draft organic social and paid advertising copy from approved direction and supplied keyword evidence, but no member selects paid media opportunities, researches bids or keyword volume, targets audiences, operates campaigns, or measures ad performance. [`seo`](../../skills/seo/SKILL.md) excludes paid search by contract |
